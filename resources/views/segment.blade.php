...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .search-bar { margin: 2.5rem 0; max-width: 720px; margin-left: auto; margin-right: auto; }
        .search-form-flex {
            display: flex; background: var(--card); border-radius: 50px; overflow: hidden;
            box-shadow: var(--shadow-md); border: 1px solid var(--border); transition: all 0.3s ease;
        }
        .search-form-flex:focus-within { border-color: var(--primary); box-shadow: 0 0 0 4px rgba(79,70,229,0.15); }
        .search-form-flex input { flex: 1; padding: 1.1rem 1.6rem; border: none; font-size: 1.05rem; outline: none; }
        .search-form-flex button { padding: 0 2rem; background: var(--primary); color: white; border: none; font-size: 1.3rem; cursor: pointer; }

        .content-wrapper { display: grid; grid-template-columns: 1fr 340px; gap: 3rem; }

        .main-segment-title { font-size: 2.3rem; font-weight: 700; text-align: center; margin-bottom: 2rem; }
        .main-segment-title span { color: var(--primary); }

        .recommendation-notif {
            background: #ecfdf5; border-left: 5px solid #10b981; padding: 1.3rem 1.6rem;
            border-radius: 10px; margin-bottom: 2rem; box-shadow: var(--shadow-sm);
        }

        .search-result-title {
            font-size: 1.65rem; font-weight: 600; margin: 3rem 0 1.5rem;
            padding-bottom: 0.7rem; border-bottom: 3px solid var(--primary); display: inline-block;
        }

        .segment-cards-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 2rem; }
        .segment-post-item { border-radius: 16px; overflow: hidden; box-shadow: var(--shadow-md); transition: all 0.4s; position: relative; }
        .segment-post-item:hover { transform: translateY(-12px); box-shadow: var(--shadow-lg); }
        .segment-item-image { width: 100%; height: 240px; object-fit: cover; }
        .segment-item-overlay { position: absolute; bottom: 0; left: 0; right: 0; padding: 2rem 1.5rem 1.5rem;
            background: linear-gradient(to top, rgba(0,0,0,0.85), rgba(0,0,0,0.4) 50%, transparent); color: white; }

        .small-post-list { display: flex; flex-direction: column; gap: 1rem; }
        .small-post-item {
            background: var(--card); padding: 1.3rem; border-radius: 12px; border: 1px solid var(--border);
            box-shadow: var(--shadow-sm); transition: all 0.3s;
        }
        .small-post-item:hover { border-color: var(--primary); transform: translateX(6px); box-shadow: var(--shadow-md); }
        .small-post-item a { color: inherit; text-decoration: none; }
        .small-post-item strong { color: var(--primary); font-weight: 600; }
        .small-post-item small { color: var(--text-muted); font-size: 0.9rem; display: block; margin-top: 0.4rem; }

        .sidebar { background: var(--card); padding: 2rem; border-radius: 16px; box-shadow: var(--shadow-md); position: sticky; top: 120px; }
        .sidebar-title { font-size: 1.5rem; font-weight: 700; margin-bottom: 1.5rem; }
        .read-more-link { display: inline-block; margin-top: 1.5rem; color:#ffffff; font-weight: 600; text-decoration: none; }
        .read-more-link:hover { color: var(--text-muted); transform: translateX(8px); }

        .no-result-box {
            background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; padding: 2rem;
            border-radius: 12px; margin-top: 2rem; text-align: center; font-size: 1.1rem; box-shadow: var(--shadow-sm);
        }

        .main-footer {
            margin-top: 5rem; padding: 2.5rem 0;color: white; text-align: center;
        }
        .footer-logo { font-size: 1.8rem; font-weight: 700; color: white; }
        .footer-copyright { opacity: 0.9; margin-top: 0.5rem; }

        @media (max-width: 1024px) { .content-wrapper { grid-template-columns: 1fr; } .sidebar { position: static; margin-top: 3rem; } }
        @media (max-width: 768px) { .segment-cards-grid { grid-template-columns: 1fr; } .main-segment-title { font-size: 1.9rem; } }
    </style>
</head>
<body>
    ...
    <main class="container">
        <div class="search-bar">
            <form action="{{ route('segments.index') }}" method="GET" class="search-form-flex">
                <input type="text" name="query" placeholder="Cari segmen, fase, materi, atau langkah..." value="{{ request('query') }}">
        ...
        <div class="content-wrapper">
            <section class="main-segment-area">
                ...
                @if (isset($recommendation) && !$query)
                    <div class="recommendation-notif">
                        <strong>Rekomendasi:</strong> {{ $recommendation }}! Segmen di bawah ini paling cocok dengan minat Anda.
                    </div>
                @endif

                <!-- 1. Segmen -->
                @if ($segments->isNotEmpty())
                    @if ($query)<h3 class="search-result-title">Segmen Ditemukan ({{ $segments->count() }})</h3>@endif
                    <div class="segment-cards-grid">
                        @foreach ($segments as $segment)
                            <a href="{{ route('course.show', ['segment' => $segment->name]) }}" class="segment-post-item">
                                <img src="{{ asset('storage/' . $segment->image_path) }}" alt="{{ $segment->name }}" class="segment-item-image" loading="lazy">
                                <div class="segment-item-overlay">
                                    <span class="category-tag">Category</span>
                                    <h3 class="segment-item-title-small">{{ $segment->name }}</h3>
                                    <div class="segment-item-description-small">{{ Str::limit($segment->description, 100) }}</div>
                                </div>
                            </a>
                        @endforeach
                    </div>
            ...
            <aside class="sidebar">
                ...
                <div class="small-post-list">
                    ...
                    @empty
                        <div class="small-post-item"><p>Belum ada materi terbaru.</p></div>
                    @endforelse
                </div>
